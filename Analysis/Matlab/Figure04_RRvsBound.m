...
for bb = 1:numBlocks
    ...
    meanBound = boundSummary(:,bb,options.snrIndex,1,1);
    stdBound = boundSummary(:,bb,options.snrIndex,1,2);
    rewardRate = coinSummary(:,bb,options.snrIndex)./600;
    ...
    if options.showSimulations
        ...
        for ss = 1:numSimBounds
            ...
            simTable = getPigeon_simulatedDataTable(specs, ...
                'generativeMean', options.generativeMeans(options.snrIndex), ...
        ...
        plot(options.simBounds, simBoundData(:,2), '-', ...
            'Color', 'r', 'LineWidth', 2);
        maxIndex = find(simBoundData(:,2)==max(simBoundData(:,2)));
        plot(options.simBounds([maxIndex maxIndex]), [-0.8 0.4], 'k--');
    end
...
for cc = 1:size(comparisons, 1)
    ...
    xs = boundSummary(:,comparisons(cc,1),options.snrIndex,1,1);
    ys = boundSummary(:,comparisons(cc,2),options.snrIndex,1,1);
    ...
    pcts = prctile(ys-xs,[25 50 75]);
    fprintf('block %d vs %d: median diff=%.2f [%.2f %.2f], p=%.2f, corr %.2f (p=%.3f)\n', ...
        comparisons(cc,1), comparisons(cc,2), pcts(2), pcts(1), pcts(3) ,ranksum(xs,ys), R, P)

    % RR comparisons
    xrs = coinSummary(:,comparisons(cc,1),options.snrIndex)./600;
    yrs = coinSummary(:,comparisons(cc,2),options.snrIndex)./600;
    [R,P] = corr(xrs, yrs, 'Type', 'Spearman');
    fprintf('   RR: corr %.2f (p=%.3f)\n', R, P)    
end
set(axs(end), 'Visible', 'off')
% 
